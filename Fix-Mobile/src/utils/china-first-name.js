let singleFirstName = `赵 钱 孙 李 周 吴 郑 王 冯 陈 褚 卫
...
之 冠 宾 香 果 蹇 称 诺 来 多 繁 戊
朴 回 毓 鉏 税 荤 靖 绪 愈 硕 牢 买
但 巧 枚 撒 泰 秘 亥 绍 以 壬 森 斋
释 奕 姒 朋 竹 百 福 言 佟 爱 年 笪
谯 哈 墨 连 赏 伯 佴 佘 牟 商 琴 后
...
况 亢 缑 帅 海 归 钦 鄢 汝 法 闫 楚
晋 督 仉 盖 逯 库 郏 逢 阴 薄 厉 稽
开 光 操 瑞 眭 泥 运 摩 伟 铁 迮 禅
鬼 死 召 有 舜 丛 岳`;

let doubleFirstName = `欧阳 太史 端木 上官 司马 东方 独孤 南宫
...
褚师 吴铭 淳于 单于 章佳 那拉 纳喇 乌雅
范姜 碧鲁 张廖 张简 图门 公叔 乌孙 完颜
马佳 佟佳 富察 费莫 东门 三小 锺离`;

let otherFirstName = `依尔根觉罗 依尔觉罗 萨嘛喇
赫舍里 额尔德特 萨克达 钮祜禄
他塔喇 喜塔腊 讷殷富察 叶赫那兰
库雅喇 瓜尔佳 舒穆禄 爱新觉罗 索绰络`;

singleFirstName = singleFirstName.replace(/\n/g, ' ').split(' ');
doubleFirstName = doubleFirstName.replace(/\n/g, ' ').split(' ');
otherFirstName = otherFirstName.replace(/\n/g, ' ').split(' ');

/**复姓在前，单姓在后 */
let chinaFirstNameList = [
  ...doubleFirstName,
  ...otherFirstName,
  ...singleFirstName
];
...
let op = {
   ...
   * @param {*} name 姓名
   */
  hiddenFirstName(name) {
    for (let firstName of chinaFirstNameList) {
      let arr = new RegExp(`^${firstName}`).exec(name);
      if (arr) {
        name = name.replace(arr[0], '**');
        break;
      }
    }
    return name;
   ...
   * @param {*} name 姓名
   */
  hiddenSecondName(name) {
    for (let firstName of chinaFirstNameList) {
      let arr = new RegExp(`^${firstName}`).exec(name);
      if (arr) {
        name = name.replace(name.substr(arr[0].length), '**');
        break;
      }
    }
    return name;
