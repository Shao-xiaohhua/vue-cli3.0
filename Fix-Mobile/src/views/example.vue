...
export default {
  ...
  methods: {
    ...mapActions(['getLogin', 'AgetCrf', 'AgetCollection']),
    onSubmit() {
      if (this.form.name && this.form.pass) {
        let pasKey = JSON.stringify(
          this.keyEncrypt(this.form.pass, this.collection)
        );
        let url = {
          _csrf: this.crfcode,
  ...
  computed: {
    ...mapGetters(['allNumber', 'crfcode', 'collection'])
  },
  created() {
    console.log('先');
  },
